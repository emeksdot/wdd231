...
const subjects = document.querySelector(".courses");
const links = subjects.querySelectorAll("a");
...
for (let index = 0; index < links.length; index++) {
  links[index].addEventListener("click", () => {
    ...
    if (links[index].innerText === "ALL") {
      createCard(courses);
      sumOfCredits = all.reduce(
        (accumulator, course) => accumulator + course.credits,
        0
      );
      creditsSpan[0].innerHTML = sumOfCredits;
    } else if (links[index].innerText === "CSE") {
      createCard(cse);
      sumOfCredits = cse.reduce(
        (accumulator, course) => accumulator + course.credits,
        0
      );
      creditsSpan[0].innerHTML = sumOfCredits;
